...
| `client` wg0 地址      |10.10.10.1/32|
| `vpn` wg0 地址         |10.20.0.0/24|
| `server` 地址          |外层：192.0.2.1:51820；隧道内网：10.20.0.0/24|
...
| 是否看到 `transfer` 计数               |是|
| client 端的 `allowed ips` 显示的是什么 |10.20.0.0/24|
...
| 项目                         | 你的填写 |
| ---------------------------- | -------- |
| `AllowedIPs` 的值            |客户端：10.20.0.0/24；服务端 (vpn)：10.10.10.1/32|
| 是否能访问 `10.20.0.2:8080`  |可以正常访问|
| `ip route` 中 wg0 相关的条目 |10.10.10.0/24 dev wg0 proto kernel scope link src 10.10.10.1 10.20.0.0/24 dev wg0 scope link|
...
| `latest handshake` 是否还在      |是，7 seconds ago，握手正常存在|
| 去 `10.20.0.0/24` 的路由是否消失 |已消失，路由表只剩 10.30.0.0/24 dev wg0，无 10.20.0.0/24 隧道路由|
| 是否还能访问 `10.20.0.2:8080`    |不能，curl 报错连接失败（7）Failed to connect|
| 失败的直接原因                   |客户端 AllowedIPs 被修改为10.30.0.0/24，系统自动删除10.20.0.0/24指向 wg0 的路由，访问 10.20 网段的流量无法从 WireGuard 隧道转发|

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？

> 答：wg show 隧道还在、业务访问失败原因
WireGuard 的握手（密钥协商）依靠底层 underlay 物理网络（192.0.2.0/24），不受 AllowedIPs 影响，所以握手可以正常完成；
而业务 IP 报文转发依赖 AllowedIPs 自动生成的内核路由，AllowedIPs 改成 10.30.0.0/24 后，10.20.0.0/24 没有隧道路由，去往 10.20.0.2 的数据包不会送入 wg0 隧道，访问失败。
隧道握手和业务传输是两套路径
握手报文：使用外层 underlay 路由（192.0.2.x 物理网），基于公网 / 底层 IP 连通；
业务数据报文：使用wg 隧道的内核路由（由 AllowedIPs 生成），走 wg 虚拟网卡封装；
二者路由逻辑、转发路径互相独立，因此握手通≠业务能通。
...
| 协议                     |UDP|ICMP|
| 是否能看到 ICMP/HTTP 内容 |看不到明文 ICMP 内容，载荷被加密|可以完整看到 ICMP 原始请求、应答报文（明文内层数据）|
| 为什么内外两层看到的不同 |underlay 是外层封装报文：WG 把原始内网 ICMP 数据包加密后，封装在 UDP+underlay IP 包头里传输|wg0 是解封装后的内层原始报文，剥离外层 UDP/IP 包头、解密后还原原生 ICMP 流量，因此源目 IP、协议、载荷全部不一样|

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？

>答：outer (underlay 抓包)：对应外层信封，只能看见信封上收发地址（192.0.2.2→192.0.2.1）、信封邮寄协议（UDP），信封内部信纸（原始 ICMP 数据）被加密看不见；
inner (wg0 抓包)：对应拆开信封后的信纸原文，能看见信纸里真实收发地址（10.10.10.1↔10.20.0.2）和完整 ICMP 通信内容；
安全意义
链路抓包只能获取外层 underlay 地址，无法窃取内层业务 IP、ICMP/HTTP 明文数据；
内层载荷全程加密传输，中间人抓 underlay 链路无法破译原始业务流量，实现传输加密、隐藏内网拓扑与业务地址，提升传输安全性。
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：路由下发：内核自动生成路由，目标网段走 wg0 隧道网卡；
入站包过滤（ACL 访问控制）：从隧道收到的 IP 包，源 IP 不在 AllowedIPs 网段内则直接丢弃。
原因：AllowedIPs优先生成内核路由决定流量转发路径，修改后对应网段隧道路由直接消失，流量无法进隧道，表现为访问不通，因此先影响路由、看似访问控制。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：握手报文：基于底层 underlay 物理网络（外层 UDP + 公网 IP）通信，仅用来交换密钥协商，不受 AllowedIPs 路由约束，只要底层192.0.2.x互通就能完成握手；
业务数据：依赖 AllowedIPs 生成的 wg 隧道路由，数据包按内核路由判断是否送入隧道封装；
两套流量转发路径、判断逻辑分离，因此握手在线不代表业务可达。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：全量路由接管：本机所有 IP 流量全部默认进入 WireGuard 隧道，所有上网、内网流量被强制转发到对端节点；
安全泄露：客户端全部网段暴露给 VPN 对端，若对端被攻陷，本机全量流量被劫持、窃听；
网络瘫痪：底层网络故障时，整机所有流量无法转发，设备断网。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能。
WireGuard 是非对称双向鉴权：隧道建立需要客户端用自身私钥签名、服务端用客户端公钥验签；攻击者只有服务端私钥，缺少 client 私钥，无法生成合法签名报文，服务端校验失败，无法冒充 client 接入隧道。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：192.0.2.2（客户端 underlay 物理 IP）；
看不到内层：WireGuard 对内层原始 IP/ICMP 载荷全程加密，底层 UDP 报文里的负载是密文，抓包只能看到外层 UDP+underlayIP 头，无法解密查看原始通信内容。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：流量加密：内层业务报文加密传输，链路抓包无法窃取 ICMP/HTTP 明文数据，防窃听；
地址隐藏：外层仅暴露 underlay 物理 IP，内网10.x.x.x网段拓扑、业务地址被封装隐藏，规避内网网段探测与扫描。
